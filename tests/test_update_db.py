...
class TestUpdateDB(DatabaseTestCase):
    ...
    def test_update_success_new_db(self):
        ...
        result = self.db.get_all_entries()
    ...
    def test_update_success_new_value(self):
        ...
        last_id = self.count_elements_json_string(self.db.get_all_entries())
        # Data to be added
        added_entries = [
            {'person_id': str(last_id - 1),
             'first_name': 'Jon',
             'last_name': 'Ferrara',
             'email': 'care@nimble.com',
             'description': 'Pioneer & creator'
             },
            {'person_id': str(last_id),
             'first_name': 'James',
             'last_name': 'McGillmore',
             'email': 'jmcgillmore@heriks.com',
             'description': None
             },
        ]
